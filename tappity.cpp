...
tappity::tappity(std::string reference) : reference(reference)
{
  userinput = "";
}
...
{
  return abs(userinput.length() - reference.length());
}
...
{
  int matching = 0;
  std::string shorter = userinput;
  std::string longer = reference;
  ...
  double acc = ((double)matching/longer.length()) * 100;
